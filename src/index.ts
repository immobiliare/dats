...
import { Socket, SocketTcp, SocketUdp } from "./socket.js";
...
class Client {
  ...
  constructor({
    ...
    this.namespace = namespace.replace(/\$\{(hostname|pid)\}/g, (_, p) =>
      String(vars[p as keyof typeof vars]),
    );
    if (this.namespace !== "") {
      this.namespace += ".";
    }
  ...
  protected buildMetric(
    ...
    const val = Types.counter === type ? value || 1 : value;

    let metric = `${this.namespace + key}:${val}|${type}`;
